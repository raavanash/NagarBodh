...
export function clusterSignals(
  ...
  const incidents: ClusteredIncident[] = groups.map((groupSignals) => {
    ...
    const isSector15 = dominantWard.toLowerCase().includes('15') || isSector15Demo;
    const historical30dCount = isSector15 ? 19 : dominantCategory === 'waterlogging' ? 8 : 4;
    const priority = calculateDeterministicPriority({
      ...
      wardName: dominantWard,
      category: dominantCategory,
      populationDensityPerSqKm: isSector15 ? 18400 : 24500,
      nearestAsset: spatialContext.nearestAsset,
      nearbyAssets: spatialContext.allNearbyAssetsWithin500m,
      rainfallMmPerHour: currentRainfallMm,
      elevationMeters: isSector15 ? 198.5 : 214,
      drainageBottleneckPercent: isSector15 ? 82 : 25,
      historical30dCount,
      slaBaselineHours: 2.5
    });

    // Incident Lifecycle Status Derivation
    // AI Engine ONLY derives up to 'dispatch_pending', 'triaged', or 'emerging'.
    // Explicit human-in-the-loop states ('approved', 'dispatched', 'on_site', 'resolving', 'resolved', 'verified') are preserved.
    const resolutionSignals = groupSignals.filter((s) => s.sentiment === 'positive_confirmation');
    let status: IncidentStatus = 'emerging';

    if (existing?.status && ['approved', 'dispatched', 'on_site', 'resolving', 'resolved', 'verified'].includes(existing.status)) {
